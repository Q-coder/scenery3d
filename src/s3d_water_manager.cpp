...
S3DWaterManager::S3DWaterManager()
{
	...
	water_material->set_flag(BaseMaterial3D::FLAG_ALBEDO_FROM_VERTEX_COLOR, true);
	water_material->set_cull_mode(StandardMaterial3D::CULL_DISABLED);
	water_material->set_flag(BaseMaterial3D::FLAG_DONT_RECEIVE_SHADOWS, true);
	water_material->set_diffuse_mode(StandardMaterial3D::DIFFUSE_LAMBERT_WRAP);
}
...
void S3DWaterManager::start_worker()
{
	if (worker_running.load()) return;
	worker_running.store(true);
	for (int i = 0; i < NUM_WORKERS; i++) {
		worker_threads.emplace_back(&S3DWaterManager::worker_func, this);
	}
}

void S3DWaterManager::stop_worker()
{
...
void S3DWaterManager::worker_func()
{
...
// ── GLB Parser ──────────────────────────────────────────────────────────────
//
// Water GLBs contain a single primitive with POSITION (vec3 float), NORMAL
...
bool S3DWaterManager::parse_glb(const std::vector<uint8_t> &data, SurfaceData &out)
{
...
void S3DWaterManager::load_manifests()
{
	manifest_loaded = true;
	manifests.clear();

	PackedStringArray paths = water_paths;
	if (paths.is_empty() && !water_path.is_empty()) {
		paths.push_back(water_path);
	}
	...
	for (int i = 0; i < paths.size(); i++) {
		...
		if (!file.is_open()) {
			UtilityFunctions::push_warning("S3DWaterManager: manifest.json not found at " + manifest_file);
			continue;
		}
		...
		if (!tiles_obj || tiles_obj->type != JsonValue::OBJECT) {
			UtilityFunctions::push_warning("S3DWaterManager: invalid manifest.json at " + manifest_file);
			continue;
		}
	...
	UtilityFunctions::print("S3DWaterManager: loaded ",
	                        (int64_t)manifests.size(), " manifest(s), ",
	                        (int64_t)total, " water tiles");
}

// ── Update ──────────────────────────────────────────────────────────────────

void S3DWaterManager::_process(double delta)
{
...
void S3DWaterManager::update_tiles(Vector3 camera_pos)
{
	...
	for (size_t gi = 0; gi < manifests.size(); gi++) {
		...
		for (auto &kv : group.entries) {
			...
			// Streams only load within load_radius_m.
			// Tiles with lake/river polygons load up to far_radius_m.
			double tile_radius = (entry.polygons > 0)
				? (double)far_radius_m : (double)load_radius_m;
			if (dist > tile_radius) continue;
	...
	for (auto &kv : tiles) {
		...
		double tile_radius = (mit->second.polygons > 0)
			? (double)far_radius_m : (double)load_radius_m;
		double de = mit->second.center_e - cam_e;
		double dn = mit->second.center_n - cam_n;
		double dist = std::sqrt(de * de + dn * dn);
		if (dist > tile_radius + (double)unload_margin_m)
...
void S3DWaterManager::process_load_results()
{
...
void S3DWaterManager::_bind_methods()
{
	ClassDB::bind_method(D_METHOD("get_active_tile_count"), &S3DWaterManager::get_active_tile_count);

	ClassDB::bind_method(D_METHOD("set_water_path", "path"), &S3DWaterManager::set_water_path);
	ClassDB::bind_method(D_METHOD("get_water_path"), &S3DWaterManager::get_water_path);
	ADD_PROPERTY(PropertyInfo(Variant::STRING, "water_path"), "set_water_path", "get_water_path");

	ClassDB::bind_method(D_METHOD("set_water_paths", "paths"), &S3DWaterManager::set_water_paths);
	ClassDB::bind_method(D_METHOD("get_water_paths"), &S3DWaterManager::get_water_paths);
	ADD_PROPERTY(PropertyInfo(Variant::PACKED_STRING_ARRAY, "water_paths"), "set_water_paths", "get_water_paths");

	ClassDB::bind_method(D_METHOD("set_origin_east", "east"), &S3DWaterManager::set_origin_east);
	ClassDB::bind_method(D_METHOD("get_origin_east"), &S3DWaterManager::get_origin_east);
	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "origin_east"), "set_origin_east", "get_origin_east");

	ClassDB::bind_method(D_METHOD("set_origin_north", "north"), &S3DWaterManager::set_origin_north);
	ClassDB::bind_method(D_METHOD("get_origin_north"), &S3DWaterManager::get_origin_north);
	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "origin_north"), "set_origin_north", "get_origin_north");

	ClassDB::bind_method(D_METHOD("set_load_radius_m", "radius"), &S3DWaterManager::set_load_radius_m);
	ClassDB::bind_method(D_METHOD("get_load_radius_m"), &S3DWaterManager::get_load_radius_m);
	ADD_PROPERTY(PropertyInfo(Variant::INT, "load_radius_m"), "set_load_radius_m", "get_load_radius_m");

	ClassDB::bind_method(D_METHOD("set_far_radius_m", "radius"), &S3DWaterManager::set_far_radius_m);
	ClassDB::bind_method(D_METHOD("get_far_radius_m"), &S3DWaterManager::get_far_radius_m);
	ADD_PROPERTY(PropertyInfo(Variant::INT, "far_radius_m"), "set_far_radius_m", "get_far_radius_m");

	ClassDB::bind_method(D_METHOD("set_vertical_offset_m", "offset"), &S3DWaterManager::set_vertical_offset_m);
	ClassDB::bind_method(D_METHOD("get_vertical_offset_m"), &S3DWaterManager::get_vertical_offset_m);
	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "vertical_offset_m"), "set_vertical_offset_m", "get_vertical_offset_m");
}

// ── Getters / Setters ───────────────────────────────────────────────────────

int S3DWaterManager::get_active_tile_count() const
{
	int count = 0;
	for (auto &kv : tiles) {
...
void S3DWaterManager::set_water_path(const String &p_path)
{
	water_path = p_path;
	manifest_loaded = false;
}
String S3DWaterManager::get_water_path() const { return water_path; }

void S3DWaterManager::set_water_paths(const PackedStringArray &p_paths)
{
	water_paths = p_paths;
	manifest_loaded = false;
}
PackedStringArray S3DWaterManager::get_water_paths() const { return water_paths; }

void S3DWaterManager::set_origin_east(double p_east) { origin_east = p_east; }
double S3DWaterManager::get_origin_east() const { return origin_east; }

void S3DWaterManager::set_origin_north(double p_north) { origin_north = p_north; }
double S3DWaterManager::get_origin_north() const { return origin_north; }

void S3DWaterManager::set_load_radius_m(int p_radius) { load_radius_m = p_radius; }
int S3DWaterManager::get_load_radius_m() const { return load_radius_m; }

void S3DWaterManager::set_far_radius_m(int p_radius) { far_radius_m = p_radius; }
int S3DWaterManager::get_far_radius_m() const { return far_radius_m; }

void S3DWaterManager::set_vertical_offset_m(double p_offset)
{
